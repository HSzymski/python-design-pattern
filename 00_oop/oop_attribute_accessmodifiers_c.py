...
public = {attrname: attrval
          for attrname, attrval in vars(DATA).items()
          if not attrname.startswith('_')}

# All protected attributes and their values
# type: dict[str,float|str]
protected = {attrname: attrval
             for attrname, attrval in vars(DATA).items()
             if attrname.startswith('_') and not attrname.startswith(f'_{DATA.__class__.__name__}__')}

# All private attributes and their values
# type: dict[str,str]
private = {attrname: attrval
           for attrname, attrval in vars(DATA).items()
           if attrname.startswith(f'_{DATA.__class__.__name__}__')}
